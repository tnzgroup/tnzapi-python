...
class SMS(ModelRequestMixin):

    _request_model_cls = SMSRequest
    ...
    def SendMessage(self, model=None, **kwargs):
        ...
        if model is not None:
            if isinstance(model, SMSRequest):
                self._data = copy.deepcopy(model)
            elif isinstance(model, dict):
                invalid = self._first_invalid_field(model)
                if invalid:
                    self._reset()
                    return SMSResponse(Result="Failed", ErrorMessage=[f"Unknown argument: {invalid}"])
                self._reset()
                self._apply_fields(model)
            else:
                self._reset()
                return SMSResponse(Result="Failed", ErrorMessage=[f"Invalid model type: {type(model).__name__}"])
        ...
        missing_destination = self._missing_destination_error(SMSResponse)
        ...
        if not (self._data.Message or self._data.TemplateID):
            self._reset()
            return SMSResponse(Result="Failed", ErrorMessage=["Missing required field: Message or TemplateID"])

        try:
            response = self.http.post("/sms", self._build_request_body())
            result = parse_response(response, SMSResponse)
        finally:
            self._reset()
    ...
    def Status(self, MessageID: str, RecordsPerPage: int = 20, Page: int = 1):
        ...
        path = f"/sms/{quote(str(MessageID), safe='')}?{urlencode(query)}"
        response = self.http.get(path)
        return parse_response(response, SMSStatus)

    @accept_legacy_kwargs({"message_id": "MessageID", "records_per_page": "RecordsPerPage", "page": "Page"})
    def Reply(self, MessageID: str, RecordsPerPage: int = 20, Page: int = 1):

        return self.Status(MessageID, RecordsPerPage, Page)

    @accept_legacy_kwargs({"message_id": "MessageID", "send_time": "SendTime"})
    def Reschedule(self, MessageID: str, SendTime: str):

        MessageID = require_id(MessageID, "MessageID")
        response = self.http.patch(f"/sms/{quote(str(MessageID), safe='')}/reschedule", {"SendTime": SendTime})
        return parse_response(response, SMSActionResult)

    @accept_legacy_kwargs({"message_id": "MessageID"})
    def Abort(self, MessageID: str):

        MessageID = require_id(MessageID, "MessageID")
        response = self.http.patch(f"/sms/{quote(str(MessageID), safe='')}/abort", {})
        return parse_response(response, SMSActionResult)
    ...
    def Received(self, TimePeriod=None, DateFrom=None, DateTo=None, RecordsPerPage: int = 20, Page: int = 1):
        ...
        if TimePeriod is not None and not (1 <= TimePeriod <= 1440):
            return SMSReceived(Result="Failed", ErrorMessage=["Invalid TimePeriod: must be between 1 and 1440 minutes"])
        ...
        response = self.http.get(f"/sms/received?{urlencode(query)}")
        return parse_response(response, SMSReceived)
